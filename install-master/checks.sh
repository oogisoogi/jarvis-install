...
if [ -n "$GET" ]; then
  ...
  #   지우기 직전 관문(reset-clean 「위 목록을 지웁니다. 되돌릴 수 없습니다.」)은 행위 시점의 정직한 고지라 그대로 둔다.
    # ★2026-09-17(977bcc1 · reinstall -Yes 실측): 재설치 한 줄은 확인 입력 없이 「목록을 보여 준 뒤」 지운다 — 사이트 문구도 그대로다.
  grep -q '삭제도 자동으로 합니다' "$GET" && grep -q '목록으로 보여 드린 뒤에 지웁니다' "$GET"; ck "[사이트] 삭제도 자동·목록을 보여 준 뒤에 지운다고 말한다" $? "사람이 모르고 누르거나 · 겁내서 안 누른다"
