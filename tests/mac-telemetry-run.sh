...
before=$(reqcount)
run_lib p1 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23
...
for r, p in zip(reqs, pts):
    ...
    if b.get("installer_version") != "0.3.23": bad.append(f"{ln}: 판번 {b.get('installer_version')}")
...
run_lib p2 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; install_id_ensure; printf "%s" "$INSTALL_ID" > "$T_SB/id"' T_URL="$URL"
...
before=$(reqcount)
run_lib p4 'HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23
...
run_lib p5 'MODE=full; INSTALLER_VERSION=0.3.23; JARVIS_PROGRESS_URL="http://127.0.0.1:9/api/progress"
...
run_lib p6 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; progress_send 2/10 start; echo $? > "$T_SB/rc"' T_URL="$URL"
...
before=$(reqcount)
run_lib p8 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; evidence_once fail; evidence_once fail; evidence_once stall' T_URL="$URL"
...
before=$(reqcount)
run_lib p8b 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; jcode J-CYS-07 "설치 실패"' T_URL="$URL"
...
before=$(reqcount)
run_lib p9 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; say "[5/10] npm ERR! install failed for hong@example.com at /Users/hongildong/x"; say "[5/10] 두 번째 Failed 줄"; say "평범한 줄"' T_URL="$URL"
python3 - "$REQ" "$before" <<'PY'
import json, sys
reqs = [json.loads(l) for l in open(sys.argv[1], encoding="utf-8")][int(sys.argv[2]):]
ev = [json.loads(r["body"]) for r in reqs if r["what"] == "progress"]
...
before=$(reqcount)
run_lib p10 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; progress_send 5/10 start; echo "$CAPTURE_REQUESTED" > "$T_SB/req"; capture_requested_run; capture_requested_run' T_URL="$URL"
...
before=$(reqcount)
run_lib p11 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23
...
before=$(reqcount)
run_lib p12 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23
...
before=$(reqcount)
run_lib p13 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; evidence_event_send slow ""; evidence_image_send installer_window "$T_JPG"; echo $? > "$T_SB/rc"' T_URL="$URL" T_JPG="$SB/big.jpg"
...
run_lib p14 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; step_baselines_update
...
run_lib p15 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; step_baselines_update; echo "$STEP_BASELINE_NOTE" > "$T_SB/r"' T_URL="$URL"
...
before=$(reqcount)
run_lib p16 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23
...
SHOT_BODY='MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23
...
run_lib p17 'MODE=full; HELP_API_URL="$T_URL"; INSTALLER_VERSION=0.3.23; RH_ID=TEST2345; RH_TMP="$T_SB/rh"; mkdir -p "$RH_TMP"
