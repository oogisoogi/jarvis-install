...
MUTANTS = [
    ...
    ("subscription-leak", PS, "    Write-Log ('login subscription: ' + (Get-LoginSubscription $authText))\n", "    Write-Log ('login subscription: ' + $authText)\n",
     "[승인 지연] 구독 종류만 적고", "emu"),
    ("closing-filter-drop", PS, "        $loudErr  = @($Error | Where-Object { -not ($_.InvocationInfo -and ([string]$_.InvocationInfo.Line -match '-(ErrorAction|EA)\\s+(SilentlyContinue|Ignore)')) })\n",
     "        $loudErr  = @($Error)\n",
    ...
    ("new-window-revert", PS, "-ArgumentList 'auth','login' -PassThru -ErrorAction Stop", "-ArgumentList 'auth','login' -NoNewWindow -PassThru -ErrorAction Stop",
     "[v0317] 로그인은 새 창에서 연다", "checks"),
