...
MUTANTS = [
    ...
    # 맥: v0324 C2(master#4c9d14e3 A) — N13 흉내 축을 대기 단계 출력으로 좁혔다 ⇒ 대기 카드에 선언 안내가 새면 좁힌 범위가 여전히 붉혀야 한다(공허 방지)
    ("mac-wait-card-redecl-inject", SH,
     "분 더 기다립니다) · 사람이 하실 일은 없습니다.\"",
     "분 더 기다립니다) · cysr 창의 master 자리에 『너는 마스터다.』 한 줄을 쳐 주십시오.\"",
     "[맥 마스터 깸·동료 늦음] 카드 문구에"),
    # 맥: v0324 C2 — 상한 소진 뒤 최종 카드(마스터 깸 갈래)의 재선언 안내 제거(복구법 없는 카드로 회귀)
    ("mac-final-card-redecl-drop", SH,
     "    say \"     마스터는 깨어 있습니다. 남은 자리를 다시 세우려면 cysr 창의 master 자리에 『너는 마스터다.』 한 줄을 다시 쳐 주십시오(이 설치 창이 아닙니다).\"\n",
     "    say \"     마스터는 깨어 있습니다.\"\n",
     "[맥 마스터 깸·동료 늦음] 상한 소진 뒤 최종 카드"),
    ...
    ("mac-app-sentence-early", SH,
     "  CYS_APP_OPENED=1\n  i=0; t0=\"$SECONDS\"",
     "  CYS_APP_OPENED=0\n  say \"     cys 앱 창을 열었습니다 — 자비스는 그 창의 master 자리에서 깨어납니다.\"\n  i=0; t0=\"$SECONDS\"",
    ...
    # N17(mac-parity-n17-lang) — 지침 템플릿의 응답 언어 줄을 한쪽에서 뺀다
    ("mp-lang-line-drop-sh", SH,
     "★상대는 우리말 사용자다 — 이 지침을 읽은 뒤의 모든 답·보고·화면 글은 우리말로 쓴다(코드·명령·파일 이름은 그대로).\n",
     "",
     "[맥동등 N17] 지침 템플릿", "checks"),
    ("mp-lang-line-drop-ps1", PS,
     "★상대는 우리말 사용자다 — 이 지침을 읽은 뒤의 모든 답·보고·화면 글은 우리말로 쓴다(코드·명령·파일 이름은 그대로).\n",
     "",
     "[맥동등 N17] 지침 템플릿", "checks"),
    # v0324 C2 — ps1 최종 카드(선언 들어감 갈래) 재선언 안내를 옛 거짓 약속으로 되돌림(흉내 러너가 이 갈래에 닿지 않아 checks 축이 유일한 그물)
    ("mp-final-card-redecl-revert-ps1", PS,
