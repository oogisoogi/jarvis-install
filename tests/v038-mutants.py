...
MUTANTS = [
 ...
 ("M57 저희 자산 404 에 원작자 판으로 되돌아간다(dbg-D5 F1 역전)", "install-master/bootstrap.sh",
  '          rm -f "$dst"\n          # ⛔저희 판 자산이 없어도',
  '          rm -f "$dst"\n          [ "$CYS_KIND" = "fork" ] && { cys_use_vendor_pin; step_download_cys; return $?; }\n          # ⛔저희 판 자산이 없어도',
  ["[전환] 저희 자산이 없으면 원작자 판으로 돌아가지 않는다(J-DL-05)", "[전환] 404 행동 시험 — 원작자 판 자리 호출 0 · J-DL-05 · rc 5"]),
 ("M58 깔린 cys 를 판 확인 없이 건너뛴다", "install-master/bootstrap.sh",
  '    if [ "$(cys_app_cdhash "$CYS_FORK_APP")" = "$CYS_FORK_CDHASH" ] && [ ! -d "$CYS_OLD_APP" ]; then',
  '    if true; then',
  ["[전환] 깔린 cys 는 판(CDHash)을 보고 건너뛴다"]),
 ("M59 격리 속성을 안 지운다", "install-master/bootstrap.sh",
  '  xattr -cr "$app" >>"$LOG_FILE" 2>&1 || true',
  '  : >>"$LOG_FILE"',
  ["[전환] 격리 지우기→서명 확인→옛 것 끄기→바꿔 넣기 순서"]),
 ("M60 서명 확인을 옛 것 끄기 뒤로 민다", "install-master/bootstrap.sh",
  '  if [ -d "$CYS_OLD_APP" ] || [ -d "$CYS_FORK_APP" ]; then\n    say "     돌고 있는 cys 가 있으면 먼저 끕니다."\n    cys_stop_old_app\n  fi\n',
  '',
  ["[전환] 격리 지우기→서명 확인→옛 것 끄기→바꿔 넣기 순서"]),
 ("M61 옛 cys 를 명령줄로 끈다", "install-master/bootstrap.sh",
  '  launchctl bootout "gui/$(id -u)/com.cysjavis.cysd" >>"$LOG_FILE" 2>&1 || true\n  for sig in TERM KILL; do',
  '  launchctl bootout "gui/$(id -u)/com.cysjavis.cysd" >>"$LOG_FILE" 2>&1 || true\n  pkill -f cys.app\n  for sig in TERM KILL; do',
  ["[전환] 옛 cys 끄기에 명령줄 축이 없다"]),
 ("M62 맥 재설치가 프로그램을 지운다", "install-master/reinstall.sh",
  'bash "$RESET_FILE" --yes $KEEP_APP_ARG\nreset_rc',
  'bash "$RESET_FILE" --yes\nreset_rc',
  ["[전환] 맥 재설치가 지우개에 --keep-app 을 넘긴다(두 자리)"]),
 ("M64 맥 재설치가 인텔에서도 프로그램을 남긴다", "install-master/reinstall.sh",
  '[ "$(uname -m)" = "arm64" ] && KEEP_APP_ARG="--keep-app"',
  'KEEP_APP_ARG="--keep-app"',
  ["[전환] 맥 재설치의 프로그램 남기기는 애플 실리콘에서만"]),
 ("M65 넣기 실패 뒤 반쪽을 안 치운다", "install-master/bootstrap.sh",
  'rm -rf "$dst"\n# **이번에 우리가 옮긴 옛 것만** 되돌린다',
  '# **이번에 우리가 옮긴 옛 것만** 되돌린다',
  ["[전환] 넣기 실패 뒤 반쪽을 치우고 이번에 옮긴 옛 것만 되돌린다"]),
 ("M66 지난 보관본도 되돌린다", "install-master/bootstrap.sh",
  '[ "$moved" = "1" ] && [ -e "$prev" ] && mv "$prev" "$dst"',
  '[ -e "$prev" ] && mv "$prev" "$dst"',
  ["[전환] 넣기 실패 뒤 반쪽을 치우고 이번에 옮긴 옛 것만 되돌린다"]),
 ("M67 자리 물음을 HEAD 로 되돌린다", "install-master/bootstrap.sh",
  "c=\"$(curl -sS -L -r 0-0 -o /dev/null -w '%{http_code}'",
  "c=\"$(curl -sSI -L -o /dev/null -w '%{http_code}'",
  ["[핀] 자리 물음은 받기와 같은 GET(첫 1바이트)으로 한다"]),
 ("M63 맥 지우개가 --keep-app 을 무시한다", "install-master/reset-clean.sh",
  '  if [ "$KEEP_APP" = "1" ]; then\n    say "  [남김] cys 프로그램',
  '  if false; then\n    say "  [남김] cys 프로그램',
  ["[전환] 맥 지우개는 --keep-app 이면 프로그램을 안 지운다"]),
]
