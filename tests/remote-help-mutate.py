...
MUTANTS = [
    ...
    ("M13 출처 헤더 제거",
     '    */ack|*/close|*/attach) [ -s "$RH_TMP/client-header" ] && extra+=(-H "@$RH_TMP/client-header") ;;',
     "    */ack|*/close|*/attach) : ;;",
     "report", "client-token"),
    ("M14 창 닫힘 트랩 제거",
    ...
    ("M17 기록 잠금 제거(외부 검토 1차: 동시 두 번 = OK,OK)",
     ...
     "  until true; do",
     # 동시 두 번(seq-lock)은 경합이 실제로 겹칠 때만 빨개진다(부하에 따라 흔들림 · r3 실측 1회 생존) ⇒ 겨냥 = 쥐어진 잠금을 확정적으로 재는 seq-lock-busy
     "seq", "seq-lock-busy"),
    ...
    ("M21 깨우기 전에 REACHED_WAKE(D1)",
     ...
     '  REACHED_WAKE=1\n  say "[9/10] 자비스를 깨웁니다."',
     "gate", "wake-failed"),
    ("M17b 오래된 잠금을 주인 확인 없이 치움(외부 검토 2차 #3)",
     '  until mkdir "$lock" 2>/dev/null; do\n    tries=$((tries + 1))',
     '  until mkdir "$lock" 2>/dev/null; do\n    [ -n "$(find "$lock" -maxdepth 0 -mtime +30s 2>/dev/null)" ] && rmdir "$lock" 2>/dev/null\n    tries=$((tries + 1))',
     "seq", "seq-lock-old"),
    ("M23 cys 종료 코드를 안 보고 surface: 글자로 성공 판정(외부 검토 2차 #2)",
     '    case "${seat_rc:-1}:$ref" in\n      0:*surface:*)',
     '    case "$ref" in\n      *surface:*)',
     "gate", "wake-seat-failed"),
